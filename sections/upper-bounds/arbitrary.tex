...
The starting point is the following lemma, adpopted from \cite[Proposition 3.1.1]{weltge2015sizes}, which states that the extension complexity behaves well for unions of polytopes:
...
\begin{definition}[Turning angle]
  ...
  In other words, it is the amount the angle at $v$ deviates from a straigt line. \\
...
% polish: remove hand-wavy footnote?
...
% polish: check last argument (pi(P') really is right by conv commuting on affine maps)
\begin{proof}
...
% polish: formulate prove for "This path can only leave through the interior of $\conv\{v_1,v_n\}$" better: Extract it or something
...
\begin{proof}
  ...
  % for any intersection (call it $I$) $f$ leftmost, $g$ is rightmost, $f_l$ and $f_r$ are left and right base edges, then we have $f_l$ and $g_r$ concurrent to outgoing arc $h$, by convexity it meets outside $v$, which fixes $h$ inside the cone spanned by $f$ and $g$. Therefore it can only go in direction of the interior of $\conv\{v_1,v_n\}$.
...
\begin{definition}[Slanted sequence]
  ...
  A \emph{counterclockwise-slanted} seqeunce is a counterclockwise sequence, if the above definition holds with ``clockwise'' replaced with ``counterclockwise''.
\end{definition}
