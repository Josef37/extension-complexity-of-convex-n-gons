...
% todo: Add connections to dependency graph: +28->25, -26->25, -26->19, +57->45, +36->22
...
It shows that the relation holds the other way around:
...
\begin{proof}[Proof outline]
  ...
  % Flow to s, which is a tree (outdeg = 1 for v!=s)
  % Lambda(u) is consecutive
  % there are at least two furthest nodes, which are consecutive
  % The turning angle of that edge is less than pi
  % Do induction: Find that triangle (with turning < pi), construct surrounding corner, build polyhedron by inductive assumption, cut that polyhedron by plane of triangle.
\end{proof}
...
\begin{lemma}\label{lemma:glueing-reformulation}
  % todo: Lemma 41. Maybe not needed (depends on chapters 9 to 11 formulation). Refer to \ref{theorem:glueing} then.
\end{lemma}

Now we find another way to determine which seqeunces are actually \emph{G-good}.
% todo: Define ray
...
\end{definition}
% todo: add image
% todo?: add observations (going around, scattered)
...
Now we have all building blocks to tackle proving Theorem~\ref{theorem:subsequence}, which states that a $\pi/6$-thin sequence $v$ with $n = 1024\tau^3 + 8\tau$ vertices it contains a subsequence $u$ with $\abs{u} \geq 4\tau^2$ and $\xc(u) \leq 12\tau$. In light of \ref{observation:limits-of-glueing} this subsequence makes optimal use of Theorem~\ref{theorem:glueing} in an asymptotical way.
...
\begin{proof}[Proof of Theorem~\ref{theorem:subsequence}]
  ...
  If Lemma~\ref{lemma:unslanted-subseqence} is applicable, $v$ contains a subsequence $u$ with $\abs{u} = (6+\delta)\tau = 4\tau^2 + 6\tau \geq 4\tau^2$ and $\xc(u) \leq 6\tau + \delta + 1 = 10\tau + 1 \leq 12\tau$.

  If Lemma~\ref{lemma:unslanted-subseqence} isn't applicable, then $v$ has a subsequence, which is slanted to the angle $\pi/3$ with tolerance $2\delta$. According to Lemma~\ref{lemma:slanted-subsequence}, such a subsequence has a subsequence $u$ with $\abs{u} \geq 0.25\delta^2 = 4\tau^2$ and $\xc(u) \leq 3\delta = 12\tau$.
\end{proof}
