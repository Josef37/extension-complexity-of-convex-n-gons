...
% todo9: choose right fraction (\frac or "/") to look nice (ctrl+F)
% todo5: add citations from original paper (to every definition, theorem, ...)
% todo2: remove some statements based on how much of chapter 9 to 11 is used
% todo3: Make sure all "cw" and "ccw" are replaced with full words. Also make sure all definitions are sound regarding cw/ccw.
% todo3: Introduce orientation of slice (left, right, ...)
% todo9: Add connections to dependency graph: +28->25, -26->25, -26->19, +57->45, +36->22
...
\end{definition}
% todo4: image of thin seqeunce, not-thin sequence, thin seqeunce to one side

% todo3: add that we normally look at clockwise sequences.
...
\end{definition}
% todo4: image of actue polyhedron and not acute polyhedron and image of main edges
...
\end{definition}
% todo4: image of polyhedron, diagram
...
\begin{proof}[Proof outline]
  ...
  % todo4: insert and reference image
...
\end{definition}
% todo4: Image of G-envelope

% todo5?: remove this observation (when reformulation is removed)
\begin{observation}
  If the sequence $v$ as in Definition~\ref{definition:g-envelope} is cw-ordered, then $v_G$ is a cw-ordered sequence with $n-2\abs{G}$ vertices.
\end{observation}
...
% todo5?: remove reformulation, when it is not used in last lemmata
Now that we have defined good seqeunces, we can reformulate Theorem~\ref{theorem:glueing} using them.

\begin{lemma}\label{lemma:glueing-reformulation}
  % todo5: Lemma 41. Maybe not needed (depends on chapters 9 to 11 formulation). Refer to \ref{theorem:glueing} then.
\end{lemma}
...
\begin{proof}
  % todo3: outline or full proof (would need Lemma 36...)
\end{proof}
...
\begin{definition}[Slanted sequence]
  Let $v=(v_1,\ldots,v_t)$ be a thin cw-sequence, ${\beta\in(0,\pi/2)}$, $\delta\geqslant 0$. We say that $v$ is \textit{cw-slanted to} an angle $\beta$ with \textit{tolerance} $\delta$ if, for all $i,\hat{\imath},\hat{\jmath}, j$ satisfying $1\leqslant i<\hat{\imath}<\hat{\jmath}<j\leqslant t$, the ray ${\rho}(v,i,\hat{\imath},k,\hat{\jmath}, j)$ satisfies $$\angle\left(\overrightarrow{v_{\hat{\jmath}}\wedge v_j},\,\,\rho(v,i,\hat{\imath},k,\hat{\jmath}, j)\right)<\beta$$ for all $k$ satisfying $\hat{\imath}<k<\hat{\jmath}$ except for at most $\delta$ such values of $k$. 
\end{definition}
% todo4: image
% todo3?: add observations (going around, scattered)
...
\end{enumerate}
We will show that for each case we can find a large enough subsequence, for which Theorem~\ref{theorem:glueing} can be applied well. This is done in the following lemmata, for which we will outline the proof.

\begin{lemma}[{\cite[Lemma 49]{shitov2020sublinear}}]\label{lemma:unslanted-subseqence}
  Let $\delta, \tau, m$ be positive integers and $n=8\tau m$. Let $\alpha,\beta$ be positive reals with $\pi/2>\beta\geqslant 2\alpha$. Assume $v=(v_1,\ldots,v_n)$ is an $\alpha$-thin sequence such that any its subsequence with $m$ points is neither cw-slanted nor ccw-slanted relative to the angle $\beta$ and tolerance $2\delta$. Then $v$ has a subsequence with $(6+\delta)\tau$ points with extension complexity not exceeding $6\tau+\delta+1$.
\end{lemma}

\begin{proof}[Proof outline]
  % todo2: formulate list
  % todo2: you would at least need Lemma 48... Maybe you can prove by image?
  \begin{itemize}
    \item Pick $4\tau$ not ccw-slanted subsequences with $m$ vertices, which we'll index by $q$ and $4\tau$ not cw-slanted subseq.s with $m$ vertices between each.
    ...
    \begin{itemize}
      ...
      \item Build $G$ out of those $\delta$ vertices, so the ray in \textbf{Lemma~48} is left and $\omega$ is right (by defining the point that preserves $\rho$ and $\omega$).
...
% todo2: State Theorem 57 and explain the key idea behind it -> Trace Chapter 10 and 11 (draw sketches to all proofs for understanding them yourself), maybe only state Lemma 55, since it comes from direct computation, give an overview of Lemma 52
\begin{lemma}[{\cite[Theorem 57]{shitov2020sublinear}}]\label{lemma:slanted-subsequence}
  Let $\delta>1$ be an integer and $n=8\delta^2$. Let $v=(v_0,\ldots,v_n)$ be a $\pi/6$-thin sequence that is cw-slanted to the angle $\pi/3$ with tolerance $2\delta$. Then $v$ has a subsequence of size at least $0.25\delta^2$ and extension complexity at most $3\delta$.
\end{lemma}

\begin{proof}
  % todo2: formulate
  % todo2: you would at least need Lemma 55 and Definition 56.
  \begin{itemize}
