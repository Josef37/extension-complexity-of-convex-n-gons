...
This allows us to split the polygon into smaller pieces with small extended formulations. Joining them together will transfer this property to the original polygon.

Now we will go on to define those smaller pieces, which we can handle well.

\begin{definition}[Turning angle]
  If $P\subset\R^2$ is a polygon, then the \textit{turning angle} at a vertex $v$ is $\pi-\angle v_-vv_+$, where $v_-$ and $v_+$ are two vertices adjacent to $v$ in $P$. The \textit{turning angle} of an edge $e$ of a polygon is the sum of the turning angles at the two endpoints of $e$.
\end{definition}
% todo?: Image
...
Based on Observation~\ref{observation:splitting} we choose to split the $n$-gon into $12$ distinct $pi/6$-thin sequences. Joining those (constantly many) sequences will not change the bound for the extension complexity of the original polygon in light of Lemma~\ref{lemma:union}.
...
\begin{proof}[Proof of \ref{theorem:xc}]
  % todo
\end{proof}
