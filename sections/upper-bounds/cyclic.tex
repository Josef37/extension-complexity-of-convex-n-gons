...
\begin{lemma}\label{lemma:shitov-lampshade}
  ...
  There are real numbers $\alpha_v>0$, $\beta_v\geq 0$ and a vertex $x_v\in X$ such that $M_{v,f}'=\alpha_v M_{v,f}-\beta_v M_{x_v,f}$ for all $f\in F'$.
...
\begin{proof}[Proof outline of Lemma~\ref{lemma:rank-of-K}]
  ...
  To show $\rank_+ K[V \setminus V^X, F^X] \leq 8$ we apply Lemma~\ref{lemma:shitov-lampshade} for each of the above cases with $V^X$ as our set of consecutive vertices ($F^X$ is the set of facets with both endpoints in $V^X$).\\
